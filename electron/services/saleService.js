...
const db = new PouchDB('sales');

// Create a sale
function createSale(saleData) {
  return db.post(saleData)
    .then(response => ({ success: true, sale: { ...saleData, _id: response.id, _rev: response.rev } }))
    .catch(error => {
...
function getSalesByPaymentMethod(date1, date2) {
  return db.find({
...
function getAverageTransactionValue(startDate, endDate) {
  return db.find({
    selector: { createdAt: { $gte: new Date(startDate), $lte: new Date(endDate) } }
  })
    .then(result => {
...
function getTopSellingItems(startDate, endDate, limit = 10) {
  return db.find({
    selector: { createdAt: { $gte: new Date(startDate), $lte: new Date(endDate) } }
  })
    .then(result => {
...
function getGrossProfitMargin(startDate, endDate) {
  return db.find({
    selector: { createdAt: { $gte: new Date(startDate), $lte: new Date(endDate) } }
  })
    .then(result => {
...
function getTotalSalesRevenueAndProfit(startDate, endDate) {
  return db.find({
    selector: { createdAt: { $gte: new Date(startDate), $lte: new Date(endDate) } }
  })
    .then(result => {
...
function getTopCustomers(startDate, endDate, limit = 10) {
  return db.find({
    selector: { createdAt: { $gte: new Date(startDate), $lte: new Date(endDate) } }
  })
    .then(result => {
...
function getAllSalesBetweenDates(startDate, endDate) {
  return db.find({
    selector: { createdAt: { $gte: new Date(startDate), $lte: new Date(endDate) } }
  })
    .then(result => {
