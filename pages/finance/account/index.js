    ...
    <Card>
      <CardHeader>
        <div className="flex justify-between items-center">
          <div>
            <CardTitle className="text-left">Account</CardTitle>
      ...
      <CardFooter className="flex justify-between items-center">
        <div className="text-base text-muted-foreground">
          Showing <strong>{indexOfFirstAccount + 1}-{Math.min(indexOfLastAccount, filteredAccounts?.length ?? 0)}</strong> of <strong>{filteredAccounts?.length ?? 0}</strong> accounts
        </div>
        <div className="flex gap-2 text-base">
          ...
          <Button
            ...
            size="sm"
            onClick={() => paginate(currentPage + 1)}
            disabled={indexOfLastAccount >= (filteredAccounts?.length ?? 0)}
          >
            Next
          </Button>
        </div>
